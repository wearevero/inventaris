<x-app-layout title="Index">
    <div class="px-20 my-10">
        <div class="flex justify-between text-center">
            <button class="px-4 uppercase rounded-lg font-space hover:bg-oranged hover:text-white border-4 border-oranged bg-white font-bold shadow-[6px_6px_0_0] shadow-oranged text-xl transition hover:shadow-none focus:outline-none focus:ring active:bg-green-500">
                <a href="{{ route('inventaris.export') }}">
                    Export
                </a>
            </button>
            <form method="GET">
                <input type="text" name="cari" value="{{ old('cari') }}" class="p-4 border-oranged text-white focus:border-oranged focus:ring-0 font-space text-center rounded-lg bg-transparent uppercase" placeholder="Cari data di sini...">
                <button value="cari" class="uppercase rounded-lg font-space mx-3 text-xl tracking-wider text-black bg-white hover:bg-oranged hover:text-white border-4 border-oranged px-2 py-2 font-bold shadow-[6px_6px_0_0] shadow-oranged transition hover:shadow-none focus:outline-none focus:ring active:bg-oranged" type="submit">Cari Data</button>
            </form>
        </div>
    ...
    <table class="border-separate text-white text-center items-center border-spacing-5 w-full align-middle border-white dark:text-white mx-auto rounded-lg table-auto my-10 border-2 border-solid">
        <thead class="">
            <tr class="font-display tracking-widest" data-aos="fade-up" data-aos-delay="1000" data-aos-anchor-placement="bottom-bottom">
